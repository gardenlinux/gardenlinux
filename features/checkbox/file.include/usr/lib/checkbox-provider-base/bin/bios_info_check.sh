...
if [ -f /sys/class/dmi/id/bios_version ]; then
    {
        ...
        echo "Product version:$(cat /sys/class/dmi/id/product_version 2>/dev/null)"
        echo "Product serial: $(cat /sys/class/dmi/id/product_serial 2>/dev/null)"
    } >> "$DETAILS_FILE"
    bios_ok="yes"
else
...
if [ -d /sys/firmware/efi/efivars ]; then
    efivar_count=$(find /sys/firmware/efi/efivars/ -maxdepth 1 2>/dev/null | wc -l)
