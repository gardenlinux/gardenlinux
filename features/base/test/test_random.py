...
def test_random(client, non_metal, non_provisioner_chroot, non_provisioner_qemu):
...
def test_random_longer(client, provisioner_chroot, provisioner_qemu):
    (exit_code, output, error) = client.execute_command("time dd if=/dev/random of=/dev/null bs=8k count=1000 iflag=fullblock", disable_sudo=True)
    """ test_random will take longer if it runs in a qemu VM
        Output should be like this:
# time dd if=/dev/random of=/dev/null bs=8k count=1000 iflag=fullblock
1000+0 records in
1000+0 records out
...
"""
    ...
    m=re.search(p_real, error)
    duration = (int(m.group(1)) * 60) + int(m.group(2))
    assert duration < 15, "Expected the test to run in less than 15 seconds"
