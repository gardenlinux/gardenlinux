...
class TestValidationErrors:
    """Tests for validation error detection and reporting."""

    def test_extract_markers_from_features_returns_validation_errors(self):
        """Should return tuple with validation errors list."""
        with patch("pathlib.Path.exists", return_value=False):
            markers, errors = coverage.extract_markers_from_features(
                Path("/nonexistent")
            )
            assert isinstance(markers, dict)
            assert isinstance(errors, list)
            assert len(errors) == 0  # No features to process

    def test_extract_markers_from_features_detects_missing_file_include_files(
        self, tmp_path
    ):
        """Should detect when YAML references non-existent file.include files."""
        # Create feature directory structure
        feature_dir = tmp_path / "features" / "testfeature"
        feature_dir.mkdir(parents=True)
        file_include_dir = feature_dir / "file.include"
        file_include_dir.mkdir()

        # Create YAML file that references a non-existent file
        yaml_file = feature_dir / "file.include.markers.yaml"
        yaml_file.write_text("""markers:
  "etc/nonexistent.conf":
    - "GL-TESTCOV-testfeature-config-nonexistent"
""")

        markers, errors = coverage.extract_markers_from_features(tmp_path)

        assert len(errors) == 1
        assert errors[0][0] == "testfeature"  # feature name
        assert errors[0][1] == "file.include"  # include type
        assert errors[0][2] == "etc/nonexistent.conf"  # file path

    def test_extract_markers_from_features_detects_missing_initrd_include_files(
        self, tmp_path
    ):
        """Should detect when YAML references non-existent initrd.include files."""
        # Create feature directory structure
        feature_dir = tmp_path / "features" / "testfeature"
        feature_dir.mkdir(parents=True)
        initrd_include_dir = feature_dir / "initrd.include"
        initrd_include_dir.mkdir()

        # Create YAML file that references a non-existent file
        yaml_file = feature_dir / "initrd.include.markers.yaml"
        yaml_file.write_text("""markers:
  "usr/bin/missing":
    - "GL-TESTCOV-testfeature-script-missing"
""")
        ...
        assert len(errors) == 1
        assert errors[0][0] == "testfeature"  # feature name
        assert errors[0][1] == "initrd.include"  # include type
        assert errors[0][2] == "usr/bin/missing"  # file path

    def test_extract_markers_from_features_no_errors_when_files_exist(self, tmp_path):
        """Should return empty errors list when all referenced files exist."""
        # Create feature directory structure
        feature_dir = tmp_path / "features" / "testfeature"
        feature_dir.mkdir(parents=True)
        file_include_dir = feature_dir / "file.include" / "etc"
        file_include_dir.mkdir(parents=True)

        # Create the actual file
        actual_file = file_include_dir / "config.conf"
        actual_file.write_text("test content")

        # Create YAML file that references the existing file
        yaml_file = feature_dir / "file.include.markers.yaml"
        yaml_file.write_text("""markers:
  "etc/config.conf":
    - "GL-TESTCOV-testfeature-config-config"
""")

        markers, errors = coverage.extract_markers_from_features(tmp_path)

        assert len(errors) == 0
        assert "testfeature" in markers

    @patch("util.coverage.extract_markers_from_features")
    @patch("util.coverage.load_feature_excludes")
    @patch("pathlib.Path.exists")
    def test_main_returns_one_on_validation_errors(
        self, mock_exists, mock_excludes, mock_extract
    ):
        """Should return 1 when validation errors are detected."""
        mock_exists.return_value = True
        mock_excludes.return_value = set()
        # Return validation errors
        mock_extract.return_value = (
            {"feature1": ["GL-TESTCOV-001"]},
            [("feature1", "file.include", "etc/missing.conf")],
        )
    ...
    @patch("pathlib.Path.exists")
    @patch("builtins.print")
    def test_main_reports_all_validation_errors_grouped_by_feature(
        self, mock_print, mock_exists, mock_excludes, mock_extract
    ):
        """Should report all validation errors grouped by feature."""
        mock_exists.return_value = True
        mock_excludes.return_value = set()
        # Return multiple validation errors across features
        mock_extract.return_value = (
            {"feature1": ["GL-TESTCOV-001"], "feature2": ["GL-TESTCOV-002"]},
            [
                ("feature1", "file.include", "etc/missing1.conf"),
                ("feature1", "file.include", "etc/missing2.conf"),
                ("feature2", "initrd.include", "usr/bin/missing"),
            ],
        )
        ...
        # Verify feature names appear in output
        output_text = " ".join(str(call) for call in mock_print.call_args_list)
        assert "feature1" in output_text
        assert "feature2" in output_text
...
class TestMainFunction:
    ...
    def test_main_returns_one_on_duplicates(
        ...
        mock_excludes.return_value = set()
        mock_extract.return_value = (
            {"feature1": ["GL-TESTCOV-001", "GL-TESTCOV-001"]},
            [],
        )
