...
1. Extracts setting IDs from features/*/ directories:
   - setting_ids.yaml files (YAML mappings)
   - file.include/* files (using setting_ids.yaml mappings)
   - initrd.include/* files (using setting_ids.yaml mappings)
...
class Config:
    ...
    setting_ids_yaml: str = "setting_ids.yaml"
...
def extract_setting_ids_from_features(
    ...
    for feature_dir in features_dir.iterdir():
        ...
        setting_ids_yaml_path = feature_dir / CONFIG.setting_ids_yaml
        setting_ids_mapping = {}

        if setting_ids_yaml_path.exists():
            try:
                setting_ids_data = (
                    yaml.safe_load(setting_ids_yaml_path.read_text()) or {}
                )
                if "setting_ids" in setting_ids_data:
                    setting_ids_mapping = setting_ids_data["setting_ids"]
            except Exception:
                pass
            # Don't extract setting IDs from the mapping structure here
            # They will be extracted when processing file.include/ and initrd.include/ files
