...
class Config:
    ...
    marker_pattern: str = rf"{marker_prefix}[a-zA-Z0-9_-]+"
    inline_comment_pattern: str = rf"^#\s+({marker_prefix}[^\s]+)"
...
def extract_markers_from_features(
    ...
    Returns:
        Tuple of (markers_by_feature, validation_errors) where:
        - markers_by_feature: Dict mapping feature name to list of markers
        - validation_errors: List of tuples (feature, include_type, file_path) for missing files
    """
    ...
    if not features_dir.exists():
        return markers_by_feature, validation_errors
    ...
    for feature_dir in features_dir.iterdir():
        ...
        if "file" in markers_mapping:
            file_section = markers_mapping["file"]
            if isinstance(file_section, dict) and "include" in file_section:
                file_list = file_section["include"]
                if isinstance(file_list, list):
                    for entry in file_list:
                        if isinstance(entry, dict) and "file" in entry:
                            file_path = entry["file"]
                            expected_file = feature_dir / "file.include" / file_path
                            if not expected_file.exists():
                                validation_errors.append(
                                    (feature, "file.include", file_path)
                                )
        if "initrd" in markers_mapping:
            initrd_section = markers_mapping["initrd"]
            if isinstance(initrd_section, dict) and "include" in initrd_section:
                initrd_list = initrd_section["include"]
                if isinstance(initrd_list, list):
                    for entry in initrd_list:
                        if isinstance(entry, dict) and "file" in entry:
                            file_path = entry["file"]
                            expected_file = feature_dir / "initrd.include" / file_path
                            if not expected_file.exists():
                                validation_errors.append(
                                    (feature, "initrd.include", file_path)
                                )
...
def find_markers_in_test_files(repo_root: Path) -> Set[str]:
    ...
    for test_file in tests_dir.rglob("test_*.py"):
        ...
        try:
            ...
            found = re.findall(CONFIG.marker_pattern, content)
...
def main():
    ...
    markers_by_feature_raw, validation_errors = extract_markers_from_features(
    ...
    if validation_errors:
        ...
        print("=" * 80)
        print(
            "The following files are referenced in markers YAML files but do not exist:\n"
        )
        # Group errors by feature for better readability
        errors_by_feature: Dict[str, List[Tuple[str, str]]] = {}
        for feature, include_type, file_path in validation_errors:
            if feature not in errors_by_feature:
                errors_by_feature[feature] = []
            errors_by_feature[feature].append((include_type, file_path))

        for feature in sorted(errors_by_feature.keys()):
            print(f"\nFeature '{feature}':")
            for include_type, file_path in sorted(errors_by_feature[feature]):
                print(f"  - {include_type}/{file_path}")
