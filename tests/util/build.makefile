...
.build/dist.tar.gz: util/build_dist.sh .build/runtime.tar.gz conftest.py $(wildcard plugins/*.py) $(wildcard test_*.py) $(wildcard */test_*.py) $(wildcard handlers/*.py) $(shell find integration -name 'test_*.py' 2>/dev/null) | .build
