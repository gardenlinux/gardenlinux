...
- Parses `file.include.markers.yaml` and `initrd.include.markers.yaml` files for file-based markers (see [Split ID Files](#fileincludeidsyaml-and-initrdincludeidsyaml-for-file-includes))
- Builds a complete inventory of all defined test coverage markers across all marker categories (see [Category Reference](#category-reference))

**Step 2: Test Scanning**

- Scans test files (`tests/**/test_*.py`) for pytest markers
- Extracts setting IDs from `@pytest.mark.testcov()` decorators (see [Tests](#tests))
