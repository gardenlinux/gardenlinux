...
- KVM: For `KVM` accel we need to run `Podman` with `--privileged` option
- NOTE: The testing SSHd expects the regular `authorized_keys` file to be named as `test_authorized_keys`

**With SSH autogeneration**
This is the default and recommended way.
```
sudo podman run -it --rm --privileged -v /boot/:/boot -v /lib/modules:/lib/modules -v `pwd`:/gardenlinux gardenlinux/base-test:dev /bin/bash
```

**Without SSH autogeneration**
Use this only when really needed.
```
sudo podman run -it --rm --privileged -v `pwd`:/gardenlinux -v /boot/:/boot -v /lib/modules:/lib/modules -v `pwd`/.build/:/build -v $HOME/.ssh:/root/.ssh -v ~/config:/config gardenlinux/base-test:`bin/garden-version` /bin/bash
```
