...
class ResetEnvVars():
    """Class containing the test for checking if sudoers defaults set
    env_reset."""
    failed_before = False
    def __new__(cls, client, features):
        ...
        test_is_disabled = utils.disabled_by(enabled_features, 'reset_env_vars')
        ...
        if not hasattr(cls, 'instance'):
            cls.instance = super(ResetEnvVars, cls).__new__(cls)
            ...
            if exit_code != 0:
                ...
                msg_err = ("For sudo the environment variables are not reset")
