...
class FindDupUIDs():
    ...
    def __new__(cls, client, features):
        ...
        test_is_disabled = utils.disabled_by(enabled_features, 'test_duplicate_uids')
        ...
        if not hasattr(cls, 'instance'):
            cls.instance = super(FindDupUIDs, cls).__new__(cls)
            ...
            for line in output.split('\n'):
                # Make sure to not split last empty newline
                if line != '':
                  line = line.split(":")
                  uid = line[2]
                  user = line[0]
            ...
            for key in match:
                if len(match[key]) > 1:
                    key = str(key)
                    msg_err = f"UID {key} is used multiple times in /etc/passwd"
