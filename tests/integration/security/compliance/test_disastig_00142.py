...
@pytest.mark.root(reason="Access to audit.log")
def test_audit_report_does_not_alter_original_audit_logs(shell, file_checksum):
    with TemporaryDirectory(prefix="pytest-disa-stig-") as tmpd:
        shutil.copy("/var/log/audit/audit.log", f"{tmpd}/audit.log")
        before_report_checksum = file_checksum(f"{tmpd}/audit.log")
        shell(f"aureport -if {tmpd}/audit.log")
        after_report_checksum = file_checksum(f"{tmpd}/audit.log")
