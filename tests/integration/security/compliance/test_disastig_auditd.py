...
def test_audit_finalize_rules_file_exists(file: File):
    """
    Verify the audit finalize rules file exists to lock the audit configuration.
    Ref: SRG-OS-000057-GPOS-00027
    """
    assert file.is_regular_file(
        "/etc/audit/rules.d/90-finalize.rules"
    ), "stigcompliance: audit finalize rules file is missing"


@pytest.mark.testcov(["GL-TESTCOV-disaSTIGmedium-config-audit-rules-service-override"])
@pytest.mark.feature("disaSTIGmedium")
def test_audit_rules_service_override_exists(file: File):
    """
    Verify the audit-rules service override exists to handle locked audit configuration.
    Ref: SRG-OS-000057-GPOS-00027
    """
    assert file.is_regular_file(
        "/etc/systemd/system/audit-rules.service.d/override.conf"
    ), "stigcompliance: audit-rules service override file is missing"
...
def test_aide_conf_monitors_audit_tools(parse_file: ParseFile):
    """
    Verify AIDE is configured to monitor the integrity of audit tools.
    Ref: SRG-OS-000278-GPOS-00108
    """
    lines = parse_file.lines("/etc/aide/aide.conf")
    assert (
        "/sbin/auditctl p+i+n+u+g+s+b+acl+xattrs+sha512" in lines
    ), "stigcompliance: AIDE is not configured to monitor audit tools"
