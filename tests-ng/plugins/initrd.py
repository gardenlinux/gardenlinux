...
class Initrd:
    ...
    def _extract_initrd_from_efi(self) -> Optional[str]:
        ...
        try:
            ...
            # Write extracted data to a temporary file
    ...
    def _get_contents(self, kernel_version: Optional[str] = None) -> List[str]:
        ...
        Returns:
            List of file paths found in the initrd.
        """
        if kernel_version is not None and kernel_version != self._kernel_version:
    ...
    def contains_module(
        ...
        pattern = re.compile(
            f"(usr/)?lib/modules/.*/kernel/.*/{re.escape(module_name)}\\.ko"
        )
    ...
    def list_modules(self, kernel_version: Optional[str] = None) -> List[str]:
        ...
        # Filter for kernel module files
        # Handles both lib/modules/ and usr/lib/modules/ prefixes
        pattern = re.compile(r"(usr/)?lib/modules/.*/kernel/.*\.ko(\.(xz|zst))?$")
