...
from .parse import SUPPORTED_FORMATS, MatchResult, Parse
...
class ParseFile:
    ...
    def match_regex(
        ...
        *,
        ...
        Example:
            >>> match = parse_file.match_regex("/var/log/boot.log", r"Boot completed in ([\\d.]+)s")
        """
        parser = self._get_parser(
            path,
            ignore_missing,
        ...
        if parser is None:
            return None
        return parser.match_regex(pattern, flags=flags)
    ...
    def list_contains_value(
        ...
        *,
        ...
        Example:
            >>> parse_file.list_contains_value("/etc/users.json", "groups", "admin")
        """
        parser = self._get_parser(
            path,
            ignore_missing,
        ...
        return parser.list_contains_value(
    ...
    def match_values(
        ...
        *,
        ...
        Args:
            ...
            ignore_comments: (optional, default=True) If True, strip comments where applicable.
            ignore_missing: (optional, default=False) If False, raise exception if file is missing.
                          If True, return MatchResult with all keys marked as missing when file is missing.
        ...
        if parser is None:
            ...
            return MatchResult(missing=list(expected.keys()), wrong={}, all_match=False)
        resolved_format = self._resolve_format(path, format)
        return parser.match_values(
    ...
    def match_keys(
        ...
        *,
        ...
        if parser is None:
            # File is missing and ignore_missing=True, mark all keys as missing
            return MatchResult(missing=key_paths, wrong={}, all_match=False)
        resolved_format = self._resolve_format(path, format)
        return parser.match_keys(
