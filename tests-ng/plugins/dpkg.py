...
class Dpkg:
    ...
    def package_is_installed(self, package: str) -> bool:
        ...
        is_installed = True
        for arch in arches:
            result = self._shell(f'dpkg --status {package}:{arch}', capture_output=True, ignore_exit_code=True)
            is_installed = is_installed and result.returncode == 0
        return is_installed

    def architecture(self) -> str:
        result = self._shell('dpkg --print-architecture', capture_output=True, ignore_exit_code=True)
        return result.stdout.strip()

    def foreign_architectures(self) -> list[str]:
        result = self._shell('dpkg --print-foreign-architectures', capture_output=True, ignore_exit_code=True)
