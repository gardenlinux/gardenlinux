...
class TestParseFileLine:
    ...
    def test_line_found(self, parse_file: ParseFile, tmp_path):
    ...
    def test_line_with_comments_json(self, parse_file: ParseFile, tmp_path):
    ...
    def test_line_with_comments_yaml(self, parse_file: ParseFile, tmp_path):
    ...
    def test_line_with_comments_ini(self, parse_file: ParseFile, tmp_path):
    ...
    def test_line_format_auto_detection(self, parse_file: ParseFile, tmp_path):
    ...
    def test_line_ignore_missing_true(self, parse_file: ParseFile, tmp_path):
    ...
    def test_line_ignore_missing_false(self, parse_file: ParseFile, tmp_path):
...
class TestParseFileLines:
    ...
    def test_lines_in_order(self, parse_file: ParseFile, tmp_path):
    ...
    def test_lines_any_order(self, parse_file: ParseFile, tmp_path):
    ...
    def test_lines_with_comments(self, parse_file: ParseFile, tmp_path):
    ...
    def test_lines_empty_list(self, parse_file: ParseFile, tmp_path):
    ...
    def test_lines_single_pattern(self, parse_file: ParseFile, tmp_path):
    ...
    def test_lines_ignore_missing(self, parse_file: ParseFile, tmp_path):
...
class TestParseFileRegex:
    """Tests for ParseFile.regex() method."""

    def test_regex_match(self, parse_file: ParseFile, tmp_path):
        """Test regex pattern matching."""
        test_file = tmp_path / "test.txt"
        test_file.write_text("Boot completed in 2.345s\n")

        match = parse_file.regex(test_file, r"Boot completed in ([\d.]+)s")
        assert match is not None
        assert match.group(1) == "2.345"

    def test_regex_no_match(self, parse_file: ParseFile, tmp_path):
    ...
    def test_regex_with_flags(self, parse_file: ParseFile, tmp_path):
        """Test regex with flags."""
        test_file = tmp_path / "test.txt"
        test_file.write_text("UPPERCASE TEXT\n")

        match = parse_file.regex(test_file, r"uppercase", flags=re.IGNORECASE)
        assert match is not None

    def test_regex_ignore_missing(self, parse_file: ParseFile, tmp_path):
        """Test ignore_missing parameter for regex()."""
        missing_file = tmp_path / "missing.txt"
        assert parse_file.regex(missing_file, r"pattern", ignore_missing=True) is None

        with pytest.raises(FileNotFoundError):
            parse_file.regex(missing_file, r"pattern", ignore_missing=False)

    def test_regex_empty_file(self, parse_file: ParseFile, tmp_path):
        """Test regex with empty file."""
        test_file = tmp_path / "test.txt"
        test_file.write_text("")
        match = parse_file.regex(test_file, r"pattern")
        assert match is None

    def test_regex_multiline_flag(self, parse_file: ParseFile, tmp_path):
        """Test regex with MULTILINE flag."""
        test_file = tmp_path / "test.txt"
        test_file.write_text("line1\nline2\n")
        match = parse_file.regex(test_file, r"^line2$", flags=re.MULTILINE)
        assert match is not None


class TestParseFileInclude:
    """Tests for ParseFile.include() method."""

    def test_include_json(self, parse_file: ParseFile, tmp_path):
        """Test include() with JSON format."""
        test_file = tmp_path / "test.json"
        test_file.write_text('{"groups": ["admin", "users", "developers"]}')

        assert parse_file.include(test_file, "groups", "admin", format="json")
        assert parse_file.include(test_file, "groups", "users", format="json")
        assert not parse_file.include(test_file, "groups", "root", format="json")

    def test_include_yaml(self, parse_file: ParseFile, tmp_path):
        """Test include() with YAML format."""
        test_file = tmp_path / "test.yaml"
        test_file.write_text("groups:\n  - admin\n  - users\n")

        assert parse_file.include(test_file, "groups", "admin", format="yaml")
        assert not parse_file.include(test_file, "groups", "root", format="yaml")

    def test_include_toml(self, parse_file: ParseFile, tmp_path):
        """Test include() with TOML format."""
        test_file = tmp_path / "test.toml"
        test_file.write_text('groups = ["admin", "users"]\n')

        assert parse_file.include(test_file, "groups", "admin", format="toml")
        assert not parse_file.include(test_file, "groups", "root", format="toml")

    def test_include_nested_path(self, parse_file: ParseFile, tmp_path):
        """Test include() with nested path."""
        test_file = tmp_path / "test.json"
        test_file.write_text('{"user": {"groups": ["admin", "users"]}}')

        assert parse_file.include(test_file, "user.groups", "admin", format="json")

    def test_include_format_auto_detection(self, parse_file: ParseFile, tmp_path):
        """Test include() with format auto-detection."""
        test_file = tmp_path / "test.json"
        test_file.write_text('{"groups": ["admin"]}')

        assert parse_file.include(test_file, "groups", "admin")

    def test_include_ignore_missing(self, parse_file: ParseFile, tmp_path):
        """Test ignore_missing parameter for include()."""
        missing_file = tmp_path / "missing.json"
        assert not parse_file.include(
            missing_file, "groups", "admin", format="json", ignore_missing=True
        )

    def test_include_empty_list(self, parse_file: ParseFile, tmp_path):
        """Test include() with empty list."""
        test_file = tmp_path / "test.json"
        test_file.write_text('{"groups": []}')
        assert not parse_file.include(test_file, "groups", "admin", format="json")

    def test_include_list_with_duplicates(self, parse_file: ParseFile, tmp_path):
        """Test include() with list containing duplicates."""
        test_file = tmp_path / "test.json"
...
class TestParseFileGetValues:
    """Tests for ParseFile.get_values() method."""

    def test_get_values_json(self, parse_file: ParseFile, tmp_path):
        """Test get_values() with JSON format."""
        test_file = tmp_path / "test.json"
        test_file.write_text('{"database": {"host": "localhost", "port": 5432}}')
    ...
    def test_get_values_json_all_match(self, parse_file: ParseFile, tmp_path):
        """Test get_values() with JSON format when all values match."""
        test_file = tmp_path / "test.json"
        test_file.write_text('{"database": {"host": "localhost", "port": 5432}}')
    ...
    def test_get_values_yaml(self, parse_file: ParseFile, tmp_path):
        """Test get_values() with YAML format."""
        test_file = tmp_path / "test.yaml"
        test_file.write_text("database:\n  host: localhost\n  port: 5432\n")
    ...
    def test_get_values_toml(self, parse_file: ParseFile, tmp_path):
        """Test get_values() with TOML format."""
        test_file = tmp_path / "test.toml"
        test_file.write_text('[database]\nhost = "localhost"\nport = 5432\n')
    ...
    def test_get_values_ini(self, parse_file: ParseFile, tmp_path):
        """Test get_values() with INI format."""
        test_file = tmp_path / "test.ini"
        test_file.write_text("[database]\nhost = localhost\nport = 5432\n")
    ...
    def test_get_values_keyval(self, parse_file: ParseFile, tmp_path):
        """Test get_values() with keyval format."""
        test_file = tmp_path / "test.env"
        test_file.write_text("DATABASE_HOST=localhost\nDATABASE_PORT=5432\n")
    ...
    def test_get_values_format_auto_detection(self, parse_file: ParseFile, tmp_path):
        """Test get_values() with format auto-detection."""
        test_file = tmp_path / "test.json"
        test_file.write_text('{"key1": "value1", "key2": "value2"}')

        result = parse_file.get_values(test_file, {"key1": "value1", "key2": "value2"})
        assert result.all_match is True

    def test_get_values_empty_dict(self, parse_file: ParseFile, tmp_path):
        """Test get_values() with empty dict."""
        test_file = tmp_path / "test.json"
        test_file.write_text('{"key": "value"}')

        result = parse_file.get_values(test_file, {}, format="json")
        assert result.all_match is True
        assert len(result.missing) == 0
        assert len(result.wrong) == 0

    def test_get_values_all_missing(self, parse_file: ParseFile, tmp_path):
        """Test get_values() when all keys are missing."""
        test_file = tmp_path / "test.json"
    ...
    def test_get_values_wrong_values(self, parse_file: ParseFile, tmp_path):
        """Test get_values() with wrong values."""
        test_file = tmp_path / "test.json"
    ...
    def test_get_values_mixed_missing_and_wrong(self, parse_file: ParseFile, tmp_path):
        """Test get_values() with mix of missing and wrong values."""
        test_file = tmp_path / "test.json"
        test_file.write_text('{"key1": "value1", "key2": "wrong"}')
    ...
    def test_get_values_ignore_missing(self, parse_file: ParseFile, tmp_path):
        """Test ignore_missing parameter for get_values()."""
        missing_file = tmp_path / "missing.json"
        result = parse_file.get_values(
            missing_file,
            {"key1": "v1", "key2": "v2"},
    ...
    def test_get_values_boolean_context(self, parse_file: ParseFile, tmp_path):
        """Test ValidationResult can be used in boolean context."""
        test_file = tmp_path / "test.json"
        test_file.write_text('{"key": "value"}')

        result_match = parse_file.get_values(test_file, {"key": "value"}, format="json")
        result_mismatch = parse_file.get_values(
    ...
    def test_get_values_wrong_list_formatting(self, parse_file: ParseFile, tmp_path):
        ...
        result_match = parse_file.get_values(
            test_file, {"key1": "value1"}, format="json"
        )
        assert result_match.wrong_list == []

    def test_get_values_wrong_list(self, parse_file: ParseFile, tmp_path):
...
class TestParseFileHasKeys:
    """Tests for ParseFile.has_keys() method."""

    def test_has_keys_json(self, parse_file: ParseFile, tmp_path):
        """Test has_keys() with JSON format."""
        test_file = tmp_path / "test.json"
        test_file.write_text('{"database": {"host": "localhost", "port": 5432}}')
    ...
    def test_has_keys_json_all_exist(self, parse_file: ParseFile, tmp_path):
        """Test has_keys() with JSON format when all keys exist."""
        test_file = tmp_path / "test.json"
        test_file.write_text('{"database": {"host": "localhost", "port": 5432}}')
    ...
    def test_has_keys_yaml(self, parse_file: ParseFile, tmp_path):
        """Test has_keys() with YAML format."""
        test_file = tmp_path / "test.yaml"
        test_file.write_text("database:\n  host: localhost\n  port: 5432\n")
    ...
    def test_has_keys_toml(self, parse_file: ParseFile, tmp_path):
        """Test has_keys() with TOML format."""
        test_file = tmp_path / "test.toml"
        test_file.write_text('[database]\nhost = "localhost"\nport = 5432\n')
    ...
    def test_has_keys_ini(self, parse_file: ParseFile, tmp_path):
        """Test has_keys() with INI format."""
        test_file = tmp_path / "test.ini"
        test_file.write_text("[database]\nhost = localhost\nport = 5432\n")
    ...
    def test_has_keys_keyval(self, parse_file: ParseFile, tmp_path):
        """Test has_keys() with keyval format."""
        test_file = tmp_path / "test.env"
        test_file.write_text("DATABASE_HOST=localhost\nDATABASE_PORT=5432\n")
    ...
    def test_has_keys_format_auto_detection(self, parse_file: ParseFile, tmp_path):
        """Test has_keys() with format auto-detection."""
        test_file = tmp_path / "test.json"
        test_file.write_text('{"key1": "value1", "key2": "value2"}')

        result = parse_file.has_keys(test_file, ["key1", "key2"])
        assert result.all_match is True

    def test_has_keys_empty_list(self, parse_file: ParseFile, tmp_path):
        """Test has_keys() with empty list."""
        test_file = tmp_path / "test.json"
        test_file.write_text('{"key": "value"}')

        result = parse_file.has_keys(test_file, [], format="json")
        assert result.all_match is True
        assert len(result.missing) == 0
        assert len(result.wrong) == 0

    def test_has_keys_all_missing(self, parse_file: ParseFile, tmp_path):
        """Test has_keys() when all keys are missing."""
        test_file = tmp_path / "test.json"
        test_file.write_text('{"other": "value"}')

        result = parse_file.has_keys(test_file, ["missing1", "missing2"], format="json")
        assert result.all_match is False
        assert "missing1" in result.missing
        assert "missing2" in result.missing

    def test_has_keys_ignore_missing(self, parse_file: ParseFile, tmp_path):
        """Test ignore_missing parameter for has_keys()."""
        missing_file = tmp_path / "missing.json"
        result = parse_file.has_keys(
