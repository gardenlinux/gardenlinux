...
class TestFileIsFile:
    """Tests for File.is_file() method."""

    def test_is_file_with_file(self, file: File, tmp_path):
        """Test is_file() with a regular file."""
        test_file = tmp_path / "test.txt"
        test_file.write_text("content")

        assert file.is_file(test_file)
        assert not file.is_file(tmp_path / "nonexistent")

    def test_is_file_with_directory(self, file: File, tmp_path):
        """Test is_file() returns False for directories."""
        test_dir = tmp_path / "testdir"
        test_dir.mkdir()

        assert not file.is_file(test_dir)

    def test_is_file_with_symlink(self, file: File, tmp_path):
        """Test is_file() with a symlink to a file."""
        test_file = tmp_path / "test.txt"
        test_file.write_text("content")
        symlink = tmp_path / "symlink.txt"
        symlink.symlink_to(test_file)

        # is_file() does not follow symlinks, so symlink should return False
        assert not file.is_file(symlink)
...
class TestFileHasUser:
    """Tests for File.has_user() method."""

    def test_has_user(self, file: File, tmp_path):
        """Test has_user() checks specific user ownership."""
        test_file = tmp_path / "test.txt"
        test_file.write_text("content")
        ...
        assert file.has_user(test_file, expected_user)
        assert not file.has_user(test_file, "nonexistent_user")

    def test_has_user_raises_file_not_found_error(self, file: File, tmp_path):
        """Test has_user() raises FileNotFoundError for non-existing path."""
        non_existent = tmp_path / "nonexistent.txt"

        with pytest.raises(FileNotFoundError):
            file.has_user(non_existent, "user")

    @pytest.mark.skipif(os.getuid() == 0, reason="Root can access any file")
    def test_has_user_raises_permission_error(self, file: File, restricted_directory):
        """Test has_user() raises PermissionError when access is denied."""
        restricted_dir, test_file = restricted_directory

        with pytest.raises(PermissionError):
            file.has_user(test_file, "user")


class TestFileHasGroup:
    """Tests for File.has_group() method."""

    def test_has_group(self, file: File, tmp_path):
        """Test has_group() checks specific group ownership."""
        test_file = tmp_path / "test.txt"
        test_file.write_text("content")
        ...
        assert file.has_group(test_file, expected_group)
        assert not file.has_group(test_file, "nonexistent_group")

    def test_has_group_raises_file_not_found_error(self, file: File, tmp_path):
        """Test has_group() raises FileNotFoundError for non-existing path."""
        non_existent = tmp_path / "nonexistent.txt"

        with pytest.raises(FileNotFoundError):
            file.has_group(non_existent, "group")

    @pytest.mark.skipif(os.getuid() == 0, reason="Root can access any file")
    def test_has_group_raises_permission_error(self, file: File, restricted_directory):
        """Test has_group() raises PermissionError when access is denied."""
        restricted_dir, test_file = restricted_directory

        with pytest.raises(PermissionError):
            file.has_group(test_file, "group")


class TestFileHasOwner:
    """Tests for File.has_owner() method."""

    def test_has_owner(self, file: File, tmp_path):
        """Test has_owner() checks specific owner (user and group)."""
        test_file = tmp_path / "test.txt"
        test_file.write_text("content")
        ...
        assert file.has_owner(test_file, expected_user, expected_group)
        assert not file.has_owner(test_file, "wrong_user", expected_group)
        assert not file.has_owner(test_file, expected_user, "wrong_group")
        assert not file.has_owner(test_file, "wrong_user", "wrong_group")

    def test_has_owner_raises_file_not_found_error(self, file: File, tmp_path):
        """Test has_owner() raises FileNotFoundError for non-existing path."""
        non_existent = tmp_path / "nonexistent.txt"

        with pytest.raises(FileNotFoundError):
            file.has_owner(non_existent, "user", "group")

    @pytest.mark.skipif(os.getuid() == 0, reason="Root can access any file")
    def test_has_owner_raises_permission_error(self, file: File, restricted_directory):
        """Test has_owner() raises PermissionError when access is denied."""
        restricted_dir, test_file = restricted_directory

        with pytest.raises(PermissionError):
            file.has_owner(test_file, "user", "group")
