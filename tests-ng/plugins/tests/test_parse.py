...
class TestParseLine:
    ...
    def test_line_with_comments_json_format(self, parse):
    ...
    def test_line_with_comments_ini_format(self, parse):
    ...
    def test_line_with_explicit_comment_char(self, parse):
    ...
    def test_line_with_multiple_comment_chars(self, parse):
...
class TestParseLines:
    ...
    def test_lines_in_order(self, parse):
    ...
    def test_lines_any_order(self, parse):
    ...
    def test_lines_with_comments(self, parse):
    ...
    def test_lines_single_pattern(self, parse):
...
class TestParseRegex:
    """Tests for Parse.regex() method."""

    def test_regex_match(self, parse):
    ...
    def test_regex_no_match(self, parse):
        """Test regex when pattern doesn't match."""
        parser = parse.from_str("Some other text\n")
        match = parser.regex(r"Boot completed in ([\d.]+)s")
        assert match is None

    def test_regex_with_flags(self, parse):
        """Test regex with flags."""
        parser = parse.from_str("UPPERCASE TEXT\n")
        match = parser.regex(r"uppercase", flags=re.IGNORECASE)
        assert match is not None

    def test_regex_empty_content(self, parse):
        """Test regex with empty content."""
        parser = parse.from_str("")
        match = parser.regex(r"pattern")
        assert match is None

    def test_regex_multiline_flag(self, parse):
        """Test regex with MULTILINE flag."""
        parser = parse.from_str("line1\nline2\n")
        match = parser.regex(r"^line2$", flags=re.MULTILINE)
        assert match is not None

    def test_regex_dotall_flag(self, parse):
        """Test regex with DOTALL flag."""
        parser = parse.from_str("line1\nline2")
        match = parser.regex(r"line1.line2", flags=re.DOTALL)
        assert match is not None


class TestParseInclude:
    """Tests for Parse.include() method."""

    def test_include_json(self, parse):
        """Test include() with JSON format."""
        parser = parse.from_str('{"groups": ["admin", "users"]}')
        assert parser.include("groups", "admin", format="json")
        assert not parser.include("groups", "root", format="json")

    def test_include_yaml(self, parse):
        """Test include() with YAML format."""
        parser = parse.from_str("groups:\n  - admin\n  - users\n")
        assert parser.include("groups", "admin", format="yaml")
        assert not parser.include("groups", "root", format="yaml")

    def test_include_toml(self, parse):
        """Test include() with TOML format."""
        parser = parse.from_str('groups = ["admin", "users"]\n')
        assert parser.include("groups", "admin", format="toml")
        assert not parser.include("groups", "root", format="toml")

    def test_include_nested_path(self, parse):
        """Test include() with nested path."""
        parser = parse.from_str('{"user": {"groups": ["admin"]}}')
        assert parser.include("user.groups", "admin", format="json")

    def test_include_invalid_format(self, parse):
        """Test include() raises error for unsupported format."""
        parser = parse.from_str("key = value\n")
        with pytest.raises(ValueError, match="Unsupported format"):
            parser.include("key", "value", format="ini")

    def test_include_path_not_found(self, parse):
        """Test include() raises error when path doesn't exist."""
        parser = parse.from_str('{"other": "value"}')
        with pytest.raises(ValueError, match="List path.*not found"):
            parser.include("groups", "admin", format="json")

    def test_include_not_a_list(self, parse):
        """Test include() raises error when path is not a list."""
        parser = parse.from_str('{"groups": "not a list"}')
        with pytest.raises(ValueError, match="is not a list"):
            parser.include("groups", "admin", format="json")

    def test_include_empty_list(self, parse):
        """Test include() with empty list."""
        parser = parse.from_str('{"groups": []}')
        assert not parser.include("groups", "admin", format="json")

    def test_include_list_with_duplicates(self, parse):
...
class TestParseGetValues:
    """Tests for Parse.get_values() method."""

    def test_get_values_json(self, parse):
    ...
    def test_get_values_json_all_match(self, parse):
    ...
    def test_get_values_yaml(self, parse):
    ...
    def test_get_values_toml(self, parse):
    ...
    def test_get_values_ini(self, parse):
    ...
    def test_get_values_keyval(self, parse):
    ...
    def test_get_values_empty_dict(self, parse):
        """Test get_values() with empty dict."""
        parser = parse.from_str('{"key": "value"}')
        result = parser.get_values({}, format="json")
        assert result.all_match is True
        assert len(result.missing) == 0
        assert len(result.wrong) == 0

    def test_get_values_all_missing(self, parse):
        """Test get_values() when all keys are missing."""
        parser = parse.from_str('{"other": "value"}')
        result = parser.get_values({"missing1": "v1", "missing2": "v2"}, format="json")
        assert result.all_match is False
        assert "missing1" in result.missing
        assert "missing2" in result.missing

    def test_get_values_wrong_values(self, parse):
        """Test get_values() with wrong values."""
        parser = parse.from_str('{"key1": "value1", "key2": "wrong"}')
        result = parser.get_values({"key1": "value1", "key2": "value2"}, format="json")
        assert result.all_match is False
        assert len(result.missing) == 0
        assert "key2" in result.wrong
        assert result.wrong["key2"] == ("value2", "wrong")

    def test_get_values_mixed_missing_and_wrong(self, parse):
    ...
    def test_get_values_invalid_format(self, parse):
        """Test get_values() raises error for invalid format."""
        parser = parse.from_str("some content")
        with pytest.raises(ValueError, match="Unsupported format"):
            parser.get_values({"key": "value"}, format="invalid")

    def test_get_values_boolean_context(self, parse):
        """Test ValidationResult can be used in boolean context."""
        parser = parse.from_str('{"key": "value"}')
        result_match = parser.get_values({"key": "value"}, format="json")
        result_mismatch = parser.get_values({"key": "wrong"}, format="json")

        # Test __bool__() method
        assert result_match  # Should be True
        assert not result_mismatch  # Should be False

    def test_get_values_wrong_list_formatting(self, parse):
        ...
        result_match = parser.get_values({"key1": "value1"}, format="json")
        assert result_match.wrong_list == []

    def test_get_values_wrong_list(self, parse):
        ...
        # Test with no wrong values
        result_match = parser.get_values({"key1": "wrong1"}, format="json")
        assert result_match.wrong_list == []
...
class TestParseHasKeys:
    """Tests for Parse.has_keys() method."""

    def test_has_keys_json(self, parse):
        """Test has_keys() with JSON format."""
        parser = parse.from_str('{"database": {"host": "localhost", "port": 5432}}')
        result = parser.has_keys(
    ...
    def test_has_keys_json_all_exist(self, parse):
        """Test has_keys() with JSON format when all keys exist."""
        parser = parse.from_str('{"database": {"host": "localhost", "port": 5432}}')
        result = parser.has_keys(["database.host", "database.port"], format="json")
        assert result.all_match is True
        assert len(result.missing) == 0
        assert len(result.wrong) == 0

    def test_has_keys_yaml(self, parse):
        """Test has_keys() with YAML format."""
        parser = parse.from_str("database:\n  host: localhost\n  port: 5432\n")
        result = parser.has_keys(["database.host", "database.port"], format="yaml")
        assert result.all_match is True

    def test_has_keys_toml(self, parse):
        """Test has_keys() with TOML format."""
        parser = parse.from_str('[database]\nhost = "localhost"\nport = 5432\n')
        result = parser.has_keys(["database.host", "database.port"], format="toml")
        assert result.all_match is True

    def test_has_keys_ini(self, parse):
        """Test has_keys() with INI format."""
        parser = parse.from_str("[database]\nhost = localhost\nport = 5432\n")
        result = parser.has_keys(["database.host", "database.port"], format="ini")
        assert result.all_match is True

    def test_has_keys_keyval(self, parse):
    ...
    def test_has_keys_empty_list(self, parse):
        """Test has_keys() with empty list."""
        parser = parse.from_str('{"key": "value"}')
        result = parser.has_keys([], format="json")
        assert result.all_match is True
        assert len(result.missing) == 0
        assert len(result.wrong) == 0

    def test_has_keys_all_missing(self, parse):
        """Test has_keys() when all keys are missing."""
        parser = parse.from_str('{"other": "value"}')
        result = parser.has_keys(["missing1", "missing2"], format="json")
        assert result.all_match is False
        assert "missing1" in result.missing
        assert "missing2" in result.missing
