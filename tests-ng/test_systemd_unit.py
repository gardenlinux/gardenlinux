...
@pytest.mark.parametrize(
    "systemd_unit,feature",
    [
        pytest.param(units[feature], feature, marks=pytest.mark.feature(feature))
...
def test_systemd_unit(systemd: Systemd, systemd_unit, feature):
    ...
    assert systemd.is_active(
        systemd_unit
    ), f"Required systemd unit {systemd_unit} for {feature} feature is not running"
