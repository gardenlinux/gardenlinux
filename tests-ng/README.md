...
```
apt-get update
apt-get install podman make curl jq unzip qemu swtpm socat retry
...
```
brew install coreutils bash gnu-sed gnu-getopt podman make curl jq unzip swtpm socat retry
```
