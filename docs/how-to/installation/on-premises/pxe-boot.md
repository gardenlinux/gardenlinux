...
- **Garden Linux build** — Image built with [`_pxe`](/reference/features/_pxe) (which includes [`_install`](/reference/features/_install)), [`baremetal`](/reference/features/baremetal), and [`server`](/reference/features/server) features, generating:
...
This requires an Ignition configuration that triggers the built-in `/opt/install/install.sh` (provided by the [`_install`](/reference/features/_install) feature). See [Configure First-Boot Provisioning](#configure-first-boot-provisioning).
...
:::info Built-in Installation Support
Garden Linux PXE images built with the [`_pxe`](/reference/features/_pxe) feature include the [`_install`](/reference/features/_install) feature, which provides `/opt/install/install.sh` for disk installation. This built-in installer can be used in three ways:

1. **Automatic with `_autoinstall`**: Build with the features `_pxe` and `_autoinstall` for automatic disk detection and installation
