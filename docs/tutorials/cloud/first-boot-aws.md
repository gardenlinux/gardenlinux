...
Garden Linux is a minimal, security-hardened Linux distribution designed for cloud and container environments. This tutorial guides you through deploying your first Garden Linux instance on [AWS](/reference/glossary#aws) EC2, from selecting an Amazon Machine Image (AMI) to connecting via SSH.
...
In the "Published Images" section on the release page, find the AMI ID for your desired [flavor](/explanation/flavors), [architecture](/reference/glossary#architecture), and [region](/reference/glossary#region). The default production flavor is `aws-gardener_prod-amd64`.
