...
The builder will be changed so that, by default, it enforces the presence of exactly one platform feature in a build. If zero or multiple platforms are specified, the build will fail. The previous behavior (allowing multiple platforms) will remain available via an explicit opt-in flag. Additionally, "bootstrap" will be treated as a platform, and support for platform variants (e.g., openstack-vmware vs openstack-baremetal) will be introduced, potentially via subplatforms or feature-based branching.
